...
# Title
title_label = tk.Label(left_frame, text="Registered Students", font=("Courier New", 26, "bold")
, bg="#c9adc4", fg="#333333")
...
treeview = ttk.Treeview(left_frame, height=13, columns=('USN', 'Name', 'Semester', 'DOB'))

# Configuring column widths and alignment
treeview.column('#0', width=50, anchor='center')  # Default column for IDs
treeview.column('USN', width=200, anchor='center')
treeview.column('Name', width=150, anchor='center')  # Added column for USN
treeview.column('Semester', width=150, anchor='center')
treeview.column('DOB', width=150, anchor='center')
...
def populate_treeview(treeview, db_config, branch_code=None):
    """
    ...
    query = f"SELECT usn, name, semester, dob FROM {table_name}"
    if where_clause:
        ...
        if conn.is_connected():
            ...
            try:
                ...
                for record in records:
                    ...
                    treeview.insert('', 'end', text=id, values=(record[0], record[1], record[2],record[3]))
                    id=id+1
...
# refresh_button = ttk.Button(left_frame, text="Refresh", command=lambda: on_branch_select(None), style="Refresh.TButton")
refresh_button = ttk.Button(
    left_frame,
    text="Refresh",
    command=lambda: on_branch_select(None, treeview, db_config, branch_combobox),
